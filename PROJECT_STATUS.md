...
new one reproduces the calendar's gaps exactly, on all three days checked. Two things that run
also showed, neither of them a timezone problem, noted here so the rows below aren't read as
covering more than they do:

- `find_free_time` took its **`FindItem` fallback**, not the free/busy path, because
  `OWAClient.user_email` had no writer at the time — so what that run verified is the
  calendar-scan branch's conversion. **Both branches have since been run post-merge**
  (2026-09-16, after the own-address change landed): the free/busy branch returns
  `11:30-12:00, 15:00-16:00, 17:00-18:00` for 2026-09-16 — the same day's gaps once recurring
  occurrences are counted, and *slot-for-slot identical* to what #602's `availabilityView`
  surface independently reports. Two surfaces that fail in opposite directions when the frame is
  wrong agreeing on one grid is the strongest confirmation this can get; rows #601/#602/#701/#702
  are `OK`. Getting the free/busy branch to run at all took a bearer-mode call first, which is
  a separate finding about the own-address change — logged in §4.
- **The two sources genuinely differ in coverage**, which is not a frame problem: the
  ...
  calendar scan doesn't — which is independent confirmation of the recurrence gap the
  own-address entry above describes, arrived at from the other side.
...
## 3. Tool inventory (63 tools across 11 modules)

### Email — [exchange_mcp/tools/email.py](exchange_mcp/tools/email.py) (18)
...
| 114 | `search_emails` | Full-text search for emails, scoped to one folder or the whole mailbox. With `search_all_folders=True`, tries the modern Substrate Search backend first (`/searchservice/api/v2/query`, `OWAClient.search_conversations_substrate` — the same endpoint behind OWA's own search box; understands the full AQS operator set mailbox-wide in one request) and only falls back to the per-folder path below on classic canary-cookie OWA (`BearerModeRequiredError`) or any other failure. That substrate path returns **per-conversation** (thread), not per-message, results — `conversation_id`/`message_count`/`unread_count`/`item_ids` cover the whole thread — flagged `used_substrate_search: true` in the response, and a bare empty result is now retried once before being trusted (see "Update 2026-09-18" below). The per-folder path tries EWS `FindItem`/`QueryString` (AQS syntax: `subject:`, `from:`, `body:`, `received:`, etc.) first, then transparently falls back to a client-side scan (reduced keyword subset: bare terms, `subject:`, `from:`, `category:`, `isread:`, `hasattachment:`) — this tenant's content index never returns AQS results, and `FindItem`'s `Traversal:"Deep"` is unsupported outright, so `search_all_folders` (when substrate isn't usable) enumerates folders via `FindFolder`/`Deep` (like `get_folders`) and searches each one `Shallow`. A keyword the fallback can't check locally (`body:`/`to:`/`cc:`/`bcc:`/`participants:`/`sent:`/`size:`/`importance:`) is dropped from the match rather than corrupting it, and named in the response's `fallback_unsupported_filters`; the fallback scan itself now stops only on a genuinely empty page or its own scan budget, never a short page, and sets `fallback_scan_limit_reached` plus a warning when the budget (not folder exhaustion) is why it stopped (see "Update 2026-09-18" below) | `tests/smoke/tests/test_search_emails.py`, `tests/unit/test_aqs_lite_fallback.py`, `tests/unit/test_substrate_search_mapping.py`, `tests/unit/test_substrate_search_retry.py` | Pending (2026-09-18) — two fixes from the same-day issue brief are unverified against a live mailbox: (1) Substrate Search's bare-empty-result retry (`OWAClient.search_conversations_substrate`, up to `_SUBSTRATE_EMPTY_RETRY_ATTEMPTS` attempts) — pinned by `test_substrate_search_retry.py` against a fake transport, but the actual indexing-lag/session-warmup race it targets has only been observed live, never reproduced in this dev environment (no `EXCHANGE_OWA_URL`); (2) the local fallback's stop condition no longer trusts `IncludesLastItemInRange` (`_local_search_fallback` in email.py), fixing a case where a default-scope `received:`/`sent:` search silently returned zero with no `fallback_unsupported_filters` and no warning even though matching items existed, because the scan stopped after a falsely-"last" first page — now surfaced as `fallback_scan_limit_reached` when the scan budget (not folder exhaustion) is why it stopped. Earlier OK (2026-09-17) — the substrate-search enhancement above (discovered via `owa-capability-discovery`, session `20260917-142611-4b65`) verified live on an isolated port/profile (port 8797, production on 8766 untouched): `search_emails(query="meeting", search_all_folders=True)` returned real conversation-level results (`used_substrate_search: true`) with correct subject/sender/date/preview. Two bugs surfaced only by this live run, neither caught by the unit-tested mapping: (1) `Cvid` (and `LogicalId`) must be a fresh real `uuid.uuid4()` per call, not the all-zeros placeholder `find_people`'s own suggestions endpoint tolerates — this endpoint's backend (`LocalShardSearch.Core`) validates it server-side, calling it "SessionId", and rejected the placeholder with HTTP 200 + `{"error": {"code": "BadRequest", ...}}`, which `search_conversations_substrate`'s `EntitySets`-parsing loop silently read as zero results; fixed by generating real GUIDs and raising on an `"error"` key in the response so a future regression surfaces instead of a confident empty count. (2) `ItemIds`/`GlobalItemIds` entries are `{"Id": ...}` dicts on the wire (same shape FindConversation uses), not the plain strings an earlier draft assumed and pinned in `test_substrate_search_mapping.py`'s fixture — fixed in `_search_mailbox_substrate` and the test alike. The verifications below predate this enhancement and cover only the per-folder EWS/local-scan path, which is unchanged. OK (2026-09-09) — single-folder AQS-empty + fallback, and `search_all_folders=True` across folders, both verified live; fixed `folder_id` always returning empty (`FindItem`'s `AdditionalProperties` needs the namespaced `item:ParentFolderId` FieldURI, not bare `ParentFolderId`) — re-verified non-empty `folder_id` live via the fallback path; re-verified live 2026-09-14 on the mcp **v2** SDK (2.2.0, streamable-http, 60 tools listed) with no behaviour change. OK (2026-09-17) — the fallback-keyword fix re-verified live on an isolated profile/port: `search_emails(query="received:>2026-01-01", folder="inbox")` returned 25 matches (was 0) with `used_local_fallback: true`, `fallback_unsupported_filters: ["received"]` and a `warning`, matching `get_emails` on the same folder | Stable |
...
| 116 | `get_email_status` | Batch-read `categories`, `flag_status`, `is_read`, `has_attachments` and `importance` for a list of item_ids, via the same narrow `IdOnly` + named-`AdditionalProperties` shape `assign_email_categories` already relies on — no body, recipients or attachments, and no `AllProperties` 500 on meeting invites. One `GetItem` request carries every `ItemId` regardless of batch size (was one `GetItem` per item_id — closed, see §4); per-item failures reported in `failed` with a stable `error_code`, same as `assign_email_categories` | — | Pending (2026-09-17) — `categories`/`flag_status` previously verified live (see the prior OK entry below), but the request-batching rewrite and the three new fields (`is_read`/`has_attachments`/`importance`) have not: the `IsRead`/`HasAttachments`/`Importance` `FieldURI` spellings and the assumption that a mixed success/error multi-`ItemId` `GetItem` response's items line up positionally with the request are both unconfirmed against a live mailbox. Earlier OK (2026-09-17) — verified live on an isolated port/profile (own port 8768, throwaway profile, production instance on 8766 untouched): 3 real Inbox items returned correct `categories`/`flag_status` (including a flagged, categorized item), and a malformed id correctly landed in `failed` with `error_code: "item_read_failed"` rather than aborting the batch | Stable |
| 117 | `sync_folder_items` | Delta sync for a folder via EWS `SyncFolderItems`: pass no `sync_state` for a first/full sync, then feed back the returned `sync_state` on later calls to get only what changed since — avoids re-listing the whole folder window on every poll. Changes are bucketed by `ChangeType` into `created`/`updated`/`deleted`/`read_flag_changed` (only `Create` observed live so far); `max_changes` clamped to `[1, 512]` (`MaxChangesReturned`). **`IncludesLastItemInRange` is absent from this backend's response even on a full page**, unlike the field this codebase relies on elsewhere (`FindItem`/`FindFolder`) — so `more_changes` here is a best-effort hint only (true when the page came back exactly `max_changes` long), never authoritative; the only trustworthy "caught up" signal is a genuinely empty change list, same rule as `get_emails`'s `Offset` paging | — | OK (2026-09-17) — verified live on an isolated port/profile (port 8769, throwaway profile, production on 8766 untouched): first call (`max_changes=5`) on a cold Inbox returned 5 real `Create` changes, a valid `sync_state`, `more_changes: true`; feeding that `sync_state` back returned 5 **different** items (no overlap with the first batch) — confirming the token genuinely advances the cursor rather than replaying. `updated`/`deleted`/`read_flag_changed` remain unverified live (no such changes occurred in this window) | Stable |
| 118 | `triage_emails` | Combined batch triage: mark_read, add_categories/remove_categories and flag_status all combine into **one** `UpdateItem` request per item (fewer round trips than calling `mark_email_read`/`set_email_flag`/`assign_email_categories`/`remove_email_categories` separately), plus an optional `move_to_folder` applied last as its own `MoveItem` request (EWS has no combined write+move action) via the same `OWAClient.resolve_folder()` gate every other folder-taking tool uses. Per-item failure isolation matches `assign_email_categories`'s convention: one item's failure lands in `failed` with a stable `error_code` and does not stop the rest, and a failed field update is not also attempted for the move (reported once, not twice). Implements issue #44 (feature request 7, 2026-09-18) | `tests/unit/test_triage_emails.py` | Pending (2026-09-18) — brand new, unverified against a live mailbox; the per-item core (`_triage_emails_apply`) is unit-tested against a fake `OWAClient` covering the combined-update shape, categories merge, per-item failure isolation, and move-skipped-after-failed-update, but no live run has exercised the real wire encoding yet | Stable |
...
| 208 | `assign_event_categories` | Add one or more categories to events, keeping any already present | `tests/smoke/tests/test_calendar_category_tagging.py` | OK (2026-09-16, verified live for issue #8) — tagged a disposable event with two categories and both appeared in `get_calendar_events` **list mode**, which is this row's own point (`categories` comes from the single CalendarView request, not a per-item `GetItem`). The reset was because `_set_event_categories`' header now carries the mailbox's own zone instead of a hardcoded UTC+3, and the check that actually retires that risk is not in the sentence above: **the event's start was unchanged by the write** (`2026-09-22T09:00:00Z` before and after). That payload sends `Categories` and no timestamps, so the zone was *expected* to be cosmetic here — this measured it instead, because if it were not, tagging an event would silently move it. Run on an isolated server and profile (port 8796); the event was cancelled afterwards and confirmed gone. — previously OK (2026-09-14, re-verified) — full round-trip on a real event against this tenant (tag → visible in `get_calendar_events` list mode → removed → original tags intact), so calendar tagging needs no Outlook COM fallback; attendees are never notified. Originally fixed by switching `_set_event_categories` to the bespoke `UpdateCalendarEvent` action captured from OWA's own web client; see "Update 2026-09-08 (continued) — fixed the category write-path" below. | Stable |
| 209 | `remove_event_categories` | Remove one or more categories from events, keeping any others present | `tests/smoke/tests/test_calendar_category_tagging.py` | OK (2026-09-16, verified live for issue #8) — the same run removed one of the two tags and the other survived (`['Prj-GECO', 'Prj-PIG']` → `['Prj-GECO']`), which is what "original tags intact" means here; the event's start was again unchanged by the write. Same isolated server, same cleanup. — previously OK (2026-09-14, re-verified) — untag verified live in the same round-trip as `assign_event_categories`; same fix as `assign_event_categories` above (shares `_set_event_categories`). | Stable |
...
| ID | Tool | Description | Automated test | Manual QA / Status | Stability |
|---|---|---|---|---|---|
| 601 | `find_free_time` | Find free slots in your own calendar within working hours, on the mailbox's own clock — free/busy (recurrence-expanding) when this mailbox's own address resolves, otherwise a calendar-folder scan; the response names both the source (`busy_source`) and the timezone it used | `tests/unit/test_availability_frame.py`, `tests/unit/test_mailbox_identity.py`, `tests/smoke/tests/test_find_free_time.py` | OK (2026-09-16, verified live post-merge on both of its data sources) — two independent 2026-09-16 changes landed here (timezone frame + own-address discovery) and the combination was run on an isolated server/profile afterwards. Busy periods are converted from UTC to the mailbox's wall clock before being compared with `start_hour`/`end_hour`, and the slots reproduce a known day's real gaps exactly on **both** branches: the `FindItem` calendar scan (`09:00-09:30, 11:30-12:00, 13:00-14:00, 15:00-16:00, 16:30-18:00` against single meetings only) and the free/busy path (`11:30-12:00, 15:00-16:00, 17:00-18:00` once recurring occurrences are counted, which is byte-identical to what #602's independent `availabilityView` surface reports for the same day — two different surfaces, one grid). The previous answer offered a slot inside a meeting and a four-hour block covering two more; table in the 2026-09-16 timezone update above. **Two conditions this depended on, both visible in the response:** `timezone.source` must not be `unknown` (on this branch the zone comes from `EXCHANGE_TIMEZONE`, since issue #8's `mailbox_timezone()` is not merged here), and `busy_source` must be `free_busy` — which, on this tenant, requires a bearer token to have been captured *before* the first availability call of the process; see the cold-start entry in §4. Previously OK (2026-09-08), against the skewed grid. **Post-merge with issue #8 (this PR):** the first of those two conditions no longer needs an operator to arrange it — `OWAClient.mailbox_timezone()` now exists, so `timezone.source` is `mailbox` rather than `env` and the zone resolves without `EXCHANGE_TIMEZONE` being set. Same resolved value on this tenant (`W. Europe Standard Time`), so the verification above still stands; what changes is that it no longer depends on a manual step. | Stable |
| 602 | `find_meeting_time` | Find common free slots across multiple attendees — tries the modern-backend `GetSchedule` GraphQL operation first, falling back to EWS `GetUserAvailability` only on classic/on-prem OWA | `tests/unit/test_availability_frame.py`, `tests/smoke/tests/test_find_meeting_time.py` | OK (2026-09-16, verified live post-merge) — both this branch's frame fix and main's `userId` payload change ran together on the isolated server; the suite passed and the day checked in detail agreed slot-for-slot with #601's free/busy answer. Verified: the `scheduleItems`/`CalendarEventArray` paths are converted to the mailbox's wall clock while the `availabilityView` path deliberately is **not** — it is already wall-clock in the requested zone, so it is *requested* in the resolved zone instead (`frame.schedule_tz_id`). This tool took that view branch live and its free windows moved exactly one hour (UTC+3 → UTC+2) onto real calendar gaps. It reports more busy time than #601 does, which is coverage rather than skew: the server-side view expands recurring masters that the item-level path doesn't return. Also unit-pinned now: converting the events while still *requesting* UTC+3 makes this tool and #601 disagree with each other, which is worse than the original bug. Not yet verified: `get_schedule` now sends this mailbox's resolved own address as the request's `userId` — the *requesting* user, which the Scheduling Assistant sends its own address for — instead of `emails[0]`, a queried attendee (see §4). The old value was accepted by this tenant, so that is a payload change on a working call rather than a fix, and it is the one thing to watch when re-running the suite; the post-merge run accepted it (the tenant answers a `GetSchedule` carrying the resolved own address as `userId` exactly as it answered one carrying an attendee's). `GetUserAvailability` still returns `{ErrorCode: 500, ExceptionName: NotImplementedException}` on this tenant and is unfixable server-side, but the Scheduling Assistant UI's own `GetSchedule` operation works correctly here (confirmed via live network capture) and returns free/busy data in the same `MergedFreeBusy`-compatible encoding; see "Update 2026-09-09 — found a working replacement for the broken GetUserAvailability path" above. Also fixes `get_meeting_stats`/`get_meeting_contacts` (below), which share the underlying helper. A prior fix (2026-09-08) to the legacy fallback's error path (`FaultMessage`→`ExceptionName` when the former is present-but-`null`) still applies to that branch. **Post-merge with issue #8 (this PR):** the zone this depends on now comes from `OWAClient.mailbox_timezone()` instead of requiring `EXCHANGE_TIMEZONE` to be set by hand (see #601). Same resolved value on this tenant, one fewer precondition. | Stable |

### Analytics — [exchange_mcp/tools/analytics.py](exchange_mcp/tools/analytics.py) (2)

| ID | Tool | Description | Automated test | Manual QA / Status | Stability |
|---|---|---|---|---|---|
| 701 | `get_meeting_stats` | Meeting-count statistics for one or more people over a date range | `tests/unit/test_availability_frame.py`, `tests/smoke/tests/test_get_meeting_stats.py` | OK (2026-09-16, live re-run post-merge with the zone resolved) — every count here is per-day (meetings/day, busiest weekday, unique days) and the event date came from a UTC timestamp, so a meeting near either end of the day was attributed to the wrong day and, for a Monday or Friday, the wrong week. Now dated in the mailbox's own zone (see #601 and the 2026-09-16 timezone update above); an unresolvable zone adds a "Day attribution may be off by one" line to the existing `warnings`. The error is bounded by one day rather than by the offset, which is why it read as slightly-off statistics rather than a fault. The same `userId` payload change as #602 reaches `get_schedule` through the shared helper and was covered by the same post-merge run; this tool's own arguments and response shape are unchanged. Previously — used to fail at the `ResolveNames` step (same `NullReferenceException` as `find_person` #401) before ever reaching availability data. `_resolve_to_email()` tries the Substrate Search path first (see #401, above) and falls back to `ResolveNames` only in classic canary-cookie auth mode. Its shared `_get_availability_events()` helper now tries `GetSchedule` first (see #602, above) and only falls back to the still-unimplemented `GetUserAvailability` on classic OWA, so this tool now returns real per-person stats without needing the `warnings` fallback in the common case — `warnings` remains for the classic-OWA/`GetUserAvailability`-failure path. **Post-merge with issue #8 (this PR):** the zone this depends on now comes from `OWAClient.mailbox_timezone()` instead of requiring `EXCHANGE_TIMEZONE` to be set by hand (see #601). Same resolved value on this tenant, one fewer precondition. | Stable |
| 702 | `get_meeting_contacts` | Weighted "who you meet with most" connection matrix from your own calendar; reports the `mailbox` it analysed | `tests/unit/test_availability_frame.py`, `tests/unit/test_mailbox_identity.py`, `tests/smoke/tests/test_get_meeting_contacts.py` | OK (2026-09-16, live re-run post-merge: 173 unique contacts, 10 returned, `mailbox` reported) — **this tool failed on every call from 2026-09-10 to 2026-09-16**, and the `OK` above predates the cause by one day: it guarded on `client.user_email`, whose writers left with the credential store, and returned `"User email not available. Call the login tool first."` — a remediation that could not work, since no `login` path wrote that field any more (see §4). Reproduced live on 2026-09-16 while re-verifying the timezone fix, then fixed by the own-address change: the address is now resolved via `OWAClient.resolve_own_mailbox()` and reported as `mailbox`, and the unresolved case still errors (this tool's *only* data source is own-mailbox free/busy, so there is nothing to degrade to) but names the reason instead of pointing at `login`. It also shares `_get_availability_events()` with #701 and so the same UTC-vs-local day attribution fix; the weighting is per meeting, not per day, so that effect is confined to meetings near a day boundary. `Stability` stays `Stable` and `KNOWN_BUGGY_TOOLS` untouched throughout: that column tracks *unfixable server-side* failures, and this was a fixable client-side one. That warning about the address's provenance fired on the live run and was correct to: the address resolved as `bearer_claim:upn`, so self-exclusion was comparing against a token claim rather than a confirmed primary SMTP address. **Fixed 2026-09-16 and re-verified live**: `resolve_own_mailbox()` no longer hands the bearer token to a first pass made *without* the configuration, so a UPN claim can no longer pre-empt the higher-authority `GetOwaUserConfiguration`. The address now resolves from the configuration — same value on this tenant — and the warning is gone. This tool is what surfaced it, because it is the only one that *compares* against the address rather than merely sending it; see §4. Note also that this tool only worked because the process had already made a bearer-mode call — see the cold-start entry in §4. Everything below still applies. — doesn't call `ResolveNames`, so it doesn't hit that bug. Its sole data source (own-mailbox availability via the shared `_get_availability_events()` helper) now tries `GetSchedule` first (see #602, above) and returns real meeting/contact data instead of the empty result `GetUserAvailability` always produced on this tenant. A real bug fixed while diagnosing this originally (2026-09-08): `_get_availability_events()` silently swallowed the `GetUserAvailability` failure (`except Exception: pass`) and never checked for an `ErrorCode` in a successfully-parsed error body either, so both this tool and `get_meeting_stats` were reporting a false-clean empty result instead of a diagnosable one — it now returns `(results, errors)`, and both tools add a `"warnings"` field when `errors` is non-empty (still relevant on classic OWA, where the legacy fallback is the only option). [test_get_meeting_contacts.py](tests/smoke/tests/test_get_meeting_contacts.py)/[test_get_meeting_stats.py](tests/smoke/tests/test_get_meeting_stats.py) include `warnings` in their recorded note when present. **Post-merge with issue #8 (this PR):** the zone this depends on now comes from `OWAClient.mailbox_timezone()` instead of requiring `EXCHANGE_TIMEZONE` to be set by hand (see #601). Same resolved value on this tenant, one fewer precondition. | Stable |
...
- ~~**`search_emails`'s `search_all_folders=True` Substrate Search path is non-deterministic: an identical query can answer a confident empty result on one call and the correct non-empty result on an immediate retry, with nothing distinguishing that from a genuine zero-match search.**~~ **Closed 2026-09-18** (same-day issue brief, item 9), unverified live. `OWAClient.search_conversations_substrate` now retries a bare empty result up to `_SUBSTRATE_EMPTY_RETRY_ATTEMPTS` times (with a short settle delay between attempts) before trusting it, the same asymmetric shape as `BrowserSession`'s `_SESSION_PROBE_ATTEMPTS`: a false empty is expensive (indistinguishable from a real zero) while a false non-empty can't happen (any non-empty answer already came from the server), so only the empty case pays for a second look. Any exception (`BearerModeRequiredError`, `SessionExpiredError`, the response's own `"error"` key) propagates immediately without retrying — only a bare empty *list* is suspect. Pinned by `tests/unit/test_substrate_search_retry.py` against a fake transport; the actual backend race (indexing lag or session warmup, going by the "self-corrects with no code change" symptom reported live) has not been reproduced or re-confirmed in this dev environment (no `EXCHANGE_OWA_URL`).
- ~~**`search_emails`'s default-scope (`search_all_folders` omitted/false) local fallback silently returns zero for a `received:`/`sent:` filter even when matching items exist, with no `fallback_unsupported_filters` and no warning at all.**~~ **Closed 2026-09-18** (same-day issue brief, item 10), unverified live. Root cause: `_local_search_fallback` in email.py trusted `IncludesLastItemInRange` to decide when its scan was complete — exactly the anti-pattern this file documents above for `sync_folder_items` and `find_emails_by_category`'s original bug, in a second implementation that had drifted from `_scan_conversations`'s canonical empty-page-only stop rule. Since `received`/`sent` are recognized filters (not in `_AQS_KNOWN_UNSUPPORTED_KEYWORDS`, promoted out once real date comparison landed 2026-09-17), the zero-with-no-signal result was explained by the scan stopping on a falsely-"last" first page, not by the filter being dropped. Fixed by removing the `IncludesLastItemInRange` trust, stopping only on a genuinely empty page or the scan budget (`max_scan`, unchanged), and advancing `offset` by the number of items actually returned rather than a fixed page size. A new `fallback_scan_limit_reached` signal (surfaced in `search_emails`'s response plus a warning) now distinguishes "the fallback's scan budget ran out before the folder did" from "scanned everything, nothing matched" — closing exactly the silent-zero gap the brief flagged as a regression from the 2026-09-17 fix (which surfaced `fallback_unsupported_filters` but not this case, since `received`/`sent` aren't dropped filters). No test file needed updating for the new 3-tuple return of `_local_search_fallback`: `tests/unit/test_aqs_lite_fallback.py` exercises `_parse_aqs_lite`/`_local_search_matches` directly and never calls `_local_search_fallback`.
- **Push/streaming notifications for new mail or calendar changes (issue #45, same-day brief item 8) — investigated, still open, blocked on a live discovery session.** Every read tool here is pull-based; `sync_folder_items` (row 117) makes polling cheap but it is still polling. The issue's own recommended first step — run `start_discovery_session`, drive OWA's own new-mail toast / calendar reminder UI by hand, and `classify_discovery_session` the capture to see whether a WebSocket, long-poll, or EWS-style push subscription actually reaches this transport — needs an authenticated mailbox and a visible interactive browser, neither of which exists in this dev environment (no `EXCHANGE_OWA_URL`, no display). That gap is exactly what `tools/discovery.py`'s module docstring calls "the only reliable way any gap in this codebase has ever been closed" (category writes #208/#209, `GetSchedule` #602, Substrate search #401 — all reverse-engineered from a live capture, none designed from documentation alone), so this item cannot be honestly implemented by guessing at a wire shape and writing code against it — that would be indistinguishable from the `AllProperties`-shape and `IncludesLastItemInRange` mistakes this file already tracks as bugs, just made in a new module instead of an old one. What *is* known without a capture: this codebase deliberately has no EWS SOAP client (`browser_session.py`'s whole reason to exist is that OWA's JSON action API needs real browser signals a hand-rolled client can't fake), so the textbook EWS answer — a `Subscribe` streaming-notification SOAP request kept open against `GetStreamingEvents` — is not a fit for this transport even if the tenant supports it; a push channel here would have to be something OWA's own web client actually uses (a WebSocket or SignalR-style hub, the same class of thing `tools/copilot.py`'s open question about Copilot's own answer-generation transport is already tracking), which is precisely what only a capture can confirm. The discovery recorder is already built for this exact investigation — it hooks `page.on("websocket")` as well as `response`/`requestfailed` (`discovery_session.py`, added after the first Copilot capture missed a socket-only answer path) — so closing this gap is "run the skill against a live tenant with someone watching for new mail," not new code.
- ~~**The address lookup and the timezone lookup each issue their own `GetOwaUserConfiguration`, and a `upn` claim outranks the configuration in practice.**~~ **Closed 2026-09-16**, two related fixes to `OWAClient`:
  - **One probe, not two.** Issue #8's halves were built in parallel and landed separately, each with its own method issuing the same request — a second round-trip on the first availability call for a blob that cannot change underneath us. `_owa_user_configuration()` is now the single, process-cached call site. The probes had been kept apart for a stated and *correct* reason (delegating the timezone probe to the identity one would make a `SessionExpiredError` indistinguishable from "this backend won't serve it", since that one swallows everything), so the delegation runs the other way: the shared fetcher surfaces every exception and `_user_configuration_or_none()` is the identity side's swallowing wrapper above it. Successes cache, failures don't — each caller already caches its own outcome. `forget_mailbox_address()` became `forget_mailbox_identity()` and clears the blob and timezone too, because clearing the address alone is *worse than nothing*: the next read re-resolves it out of the previous account's still-cached blob and the stale address returns looking freshly probed. Only a request-count assertion catches that.
  - **A `upn` claim no longer pre-empts the configuration.** `resolve_mailbox_address()` orders the signals correctly and `test_resolution_priority` pinned it, but `resolve_own_mailbox()` handed the bearer token to a *first* pass made without the configuration — so on every bearer-mode session the claim answered and the higher-authority `GetOwaUserConfiguration` was never fetched. Observed live as `get_meeting_contacts` resolving from `bearer_claim:upn` and emitting the "you may appear in your own ranking" warning on a mailbox whose configuration answers fine; the token is now withheld from that pass, and the claim stays the last resort. Worth recording as a testing lesson rather than just a bug: **a pure-function test can pin a priority order and still miss a caller that never supplies the higher-priority input.** Both are covered in `tests/unit/test_mailbox_identity.py`.
...
- **`get_emails` still unreachable past a server that ignores `Offset` outright (#101,
  follow-up). Fixed 2026-09-17, unit-tested only — not yet verified live.** The 2026-09-11
  fix above made `_scan_conversations` *detect* `pagination_offset_unsupported` (a witness
  row proving the server answered a deep request from folder row 0 again), but detection
  alone left the caller's data unreachable at any depth: `get_emails(offset=80)` in a folder
  that genuinely had more conversations returned `[]` with that error code instead of the
  rows. `_page_conversations` now retries once, via exactly the scan-from-zero-skip-client-side
  technique `unread_only` already uses, the moment the direct-offset attempt reports that
  error. This recovers the realistic, milder quirk — a server that honours `Offset` within its
  own cached window but resets to row 0 for a single deep jump past it, which still honours the
  smaller, incrementally-increasing offsets the fallback's own scan issues — while correctly
  continuing to report `pagination_offset_unsupported` (not looping, not returning wrong data)
  for the one case nothing client-side can fix: a server that ignores `Offset` on *every*
  request, including the fallback's own. The fallback is a real scan, not a magic escape hatch:
  it is still bounded by `_CONV_MAX_SCAN` and reports `pagination_scan_limit_reached` rather
  than answering wrong past the cap. `tests/unit/test_conversation_paging.py` models both cases
  with a fake transport (`offset_cap` for the recoverable quirk, `honour_offset=False` for the
  unrecoverable one) — the real backend's actual behaviour under a cold deep jump is unknown,
  so this is a plausible, not a confirmed, fix.
  Same call also gained a `fields` param for `ids_only=True` (comma-separated subset of
  `item_id`/`conversation_id`/`date`/`subject`), because `ids_only(limit=500)` with every field
  returns a payload observed around 150KB — large enough to exceed some MCP hosts' own
  max-result-token limit before the caller ever sees `pagination`'s "there is more" signal.
  An unknown field name is reported as an error rather than silently dropped (same reasoning as
  `search_emails`' AQS-lite fallback's `dropped` keywords, immediately below); `fields` given
  without `ids_only=True` is a non-fatal warning, not a failure, since there's nothing to narrow
  there. Covered by the new `tests/unit/test_ids_only_fields.py` (7 cases, no mailbox); the
  payload-size benefit itself is unverified live.
...
- ~~**Reminder times are written in a hardcoded timezone, and for tasks that's now
  ...
  **Live run 2026-09-16** (isolated port 8771 + its own `.browser-profile-tz`, so the always-on
  ...
  #501, #601, #1003 and #1004 went back to `OK` on the strength of that run. **Updated after
  merging main:** #601, #602, #701 and #702 are now `OK` too — not from this branch's run but
  from the post-merge live verification recorded on their own rows, which exercised the
  availability tools with the zone resolved. That leaves **four** rows `Pending`, all of them
  the calendar **write** path: #202 `create_meeting`, #203 `update_meeting`, and #208/#209 the
  category write-path.

  Those four are blocked on one thing, and it is not doubt about the fix: both suites covering
  them (`test_calendar_lifecycle`, `test_calendar_category_tagging`) require
  `EXCHANGE_SMOKE_SELF_EMAIL`, which is deliberately not in the source (public repo — see
  `tests/smoke/config.py`) and was not set for any run here. So what remains unverified is
  narrow and specific: that the calendar write actions (`CreateCalendarEvent`,
  `UpdateCalendarEvent`) accept the resolved zone and store the intended wall-clock time.
  Note that `discover_self_email()`'s Sent-Items fallback is *not* an acceptable substitute
  without the mailbox owner's say-so — `require_self_email()` is strict by design because these
  suites send mail and create calendar items.
...
- ~~**`find_free_time`/`find_meeting_time` compare naive-UTC busy periods against local
  ...
  **Verified live post-merge, on both data sources** (2026-09-16, isolated server/profile):
  the calendar-scan branch and the free/busy branch each reproduce a known day's real gaps, and
  the free/busy answer is slot-for-slot identical to what #602's independent `availabilityView`
  surface reports for that day — two different surfaces agreeing on one grid is the strongest
  confirmation available here, because they are wrong in *different* directions when the frame
...
- ~~**`resolve_own_mailbox()` caches a cold-start failure, so the first tool call of a process
  ...
  **Closed 2026-09-16.** `mailbox_identity.is_retryable()` separates "nothing had answered yet"
  ...
  degraded, recurrence-blind source. Make **any** bearer-mode call first (`find_person` will do)
  and the very next `find_free_time` on the same server resolves the address from
  `bearer_claim:upn` and reports `busy_source: free_busy`. Same profile, same signed-in session,
  same code: only the call order differed.
  The likely mechanism, stated as inference rather than fact: all three identity signals
  (`x-anchormailbox`, the config response, the JWT claims) exist only once a **bearer token has
  been captured**, and this codebase mints one lazily — a substrate call needs it, ordinary
  `service.svc` calls do not. `check_session` first did *not* help, which is what points at the
  bearer capture rather than at "any request warms it". Because the resolution is cached
  including its failures — a deliberate choice, so an identity probe can't add a request to
  every availability call forever — that first miss is permanent for the process.
  Two things make this worth fixing rather than documenting: the degraded source is *wrong*
  rather than thinner (it reports booked recurring time as free, which is the bug the
  own-address change was made to close), and an `stdio` server is a fresh process **per client
  session**, so the cold path is the common one there, not an edge case. The narrow fix is to
...
- **`get_email_status` #116 batching closed, but unverified live.** Added 2026-09-17 as a
  per-item `GetItem` (one request per item_id) to give a triage pass a way to read just
  `categories` + `flag_status` instead of `get_email`'s `AllProperties` read or
  `get_emails(include_body=True)`'s per-conversation `GetItem`; reworked the same day into a
  single `GetItem` carrying every `ItemId` in `_get_items_status`, plus three new fields
  (`is_read`, `has_attachments`, `importance`) read via the same call. This was the first
  `GetItem` call site in this file to send several `ItemIds` at once — every other one
  (`_get_item_categories`, `_get_item_details`, `get_email_links`) requests exactly one, and
  this backend has surprised the project before on things adjacent to it (`AllProperties`
  500ing mid-response serialization, `MaxEntriesReturned` not being honoured) — so two
  assumptions remain unconfirmed against a live mailbox: the `IsRead`/`HasAttachments`/
  `Importance` `FieldURI` spellings, and that a mixed success/error batch's items really do come
  back positionally matching the request order rather than needing to be matched by `ItemId`
  in the response itself. Wrong attribution here would silently write one item's status onto
  another in a caller that merges results against `item_ids` by position. `Pending` in the
  table above until a live batch (including at least one bad id, to exercise the mixed
  success/error case) confirms both.
