...
- `EXCHANGE_TIMEZONE` — (optional) Windows (`W. Europe Standard Time`) or IANA (`Europe/Rome`) timezone id for the mailbox's own free/busy times. Normally unnecessary — it is discovered from `GetOwaUserConfiguration` — but it is the **one source that outranks the server**, because every other step in the chain is something we read, and a backend reporting the wrong zone would otherwise leave no lever at all. An unreadable value is reported in the tools' `timezone` block and ignored, not applied: one typo must not silently downgrade a mailbox whose real zone was available. See "Timezones" below.
...
- `exchange_mcp/` — MCP server package (60 tools)
  ...
  - `mailbox_timezone.py` — The mailbox's own timezone and the **single** place a UTC instant becomes a wall-clock number: `MailboxTimezone` (three input-named conversions, `wire_id`, a reported `source`/`warning` degradation chain), the CLDR Windows→IANA table, and the `GetOwaUserConfiguration` parser. Pure, no Playwright, unit-testable — see "Timezones" below.
...
**Timezones — every wall-clock number is mailbox-local, and `mailbox_timezone.py` is the only converter.** Every busy-period source hands back a **UTC instant stripped to a naive datetime** (`OWAClient._parse_schedule_dt` documents the convention). That is correct for anything that only compares two instants, and wrong the moment a wall-clock number joins the arithmetic — which is what shipped as PROJECT_STATUS.md #601/#602: `find_free_time`/`find_meeting_time` subtracted naive-UTC busy periods from a 9-to-18 *local* working-day window, so every reported slot was out by the mailbox's UTC offset (2h on a W. Europe mailbox in DST). Fixed 2026-09-16. What to know before touching a datetime here:

- **Convert at the module edge, never inside the arithmetic.** `_find_free_slots` is a pure function whose four datetime inputs must share one frame and whose signature cannot say which, so `tools/availability.py` converts as soon as it parses and declares "every datetime in this module is naive mailbox-local wall clock" as its invariant. Do **not** give `_find_free_slots` a timezone argument: a second place that knows about timezones is a second place they can disagree.
- **The conversions are named after the frame of their *input***, because that is the fact a call site can get wrong: `from_utc()` (a UTC instant), `from_wire_wallclock()` (wall clock in the timezone we *asked the server for*), `from_wire_timestamp()` (an EWS value that decides between those two off its own `tzinfo`). This is not stylistic — `GetSchedule` returns `scheduleItems` as UTC **and** `availabilityView` as wall clock in the requested `tz_id`, i.e. two frames in one response, and #602 was merging them untouched. Three names make that mixture read as obviously wrong instead of as symmetry.
- **`wire_id` couples the request to the conversion**, so they cannot drift: it is the mailbox's own Windows id when known (what OWA's own client sends — and it makes `from_wire_wallclock` the *identity*, so the path that used to be wrong now does no arithmetic at all), else `"UTC"` and we convert everything ourselves. A window sent on the wire must go through `to_wire_wallclock()`. Never hardcode a timezone id in a payload: `"Russian Standard Time"` was a stray UTC+3 default that reached nine call sites across five modules before anyone noticed.
- **The degradation chain is the contract and it is reported, not silent.** `environment` (`EXCHANGE_TIMEZONE`, the operator override — it outranks the server on purpose, and a bad value warns and falls through rather than being applied) → `mailbox_configuration` → `host_local` → `utc` (identity, the pre-fix behaviour, as the floor), with `source`/`warning` surfaced in the tools' `timezone` block. Nothing in it raises: a timezone probe that fails must not take `find_free_time` offline, since it was useful-but-shifted before and useful-with-a-warning beats an exception. Equally, only a positively-determined timezone is used — every undecidable case degrades rather than guessing, because a fabricated offset is worse than a documented absent one. Same doctrine as `profile_lock.py`.
- The probe is `OWAClient.mailbox_timezone()` — `GetOwaUserConfiguration`, once per process, cached, and allowed to fail quietly. It reads the **same response** as `resolve_own_mailbox()` above, memoised in `_get_owa_user_configuration()` so two per-mailbox facts cost one round-trip; `forget_mailbox_address()` drops both, since an account switch changes the zone as well as the address. Add new timezone ids to `WINDOWS_TO_IANA` and new response spellings to `TZ_ID_KEYS`, not to the walker, and cover them in `tests/unit/test_mailbox_timezone.py`.
- **The classic-OWA `GetUserAvailability` branches cannot be reached from any live run** (it answers a permanent `NotImplementedException` here, #602), so they are covered by `tests/unit/test_availability_frames.py` instead — a fake transport that asserts the *payload* (`TimeZoneContext` id, `TimeWindow`) as well as the parsed frame, the same technique as `test_folder_resolution.py`. Any change to a busy-period parser or to a wire timezone belongs there: the guard that matters is that an *unqualified* `CalendarEvent` time is already wall clock in the `TimeZoneContext` we sent, so `from_utc()` on that path double-shifts it into a bug indistinguishable from #601. That suite's mutation-checked assertions reproduce the original bug's exact numbers when the conversion is reverted.
- **Build every request header with `OWAClient.request_header(version)`, never by hand.** That is the rule that finished issue #8 (2026-09-16): the stray `"Russian Standard Time"` reached *nine* call sites across five modules, and two of them — `tasks.py` and `folders.py` — held it in a **module-level constant**, which no per-call value can reach. That is what made a one-literal bug a codebase-wide change. `mailbox_timezone.request_header()` is now the only place a `TimeZoneContext` is constructed, and `tests/unit/test_request_headers.py` asserts that structurally (plus #8's own acceptance criterion: no hardcoded `TimeZoneDefinition` id anywhere in the package) — so the *next* module to grow a private copy fails a test rather than shipping.
- **Two headers deliberately carry no `TimeZoneContext`, and both look like oversights.** `tasks.py`'s `_READ_HEADER`: task `DueDate`/`StartDate` are stored as UTC midnight and written `Z`-qualified, so a context on a *read* would have Exchange convert them and return the previous day for any mailbox west of UTC — the classic off-by-one-day task date. Making the read and write headers symmetrical is the obvious tidy-up and it breaks dates silently. And `calendar._resolve_attendee`: `ResolveNames` has no timestamps, so a context is meaningless there (the first pass of the #8 migration added one by accident, via a blanket replace). Both are pinned in `test_request_headers.py`; pass `with_timezone=False` when you genuinely need the omission.
- **A reminder and its header are one mechanism.** `tasks.py`'s `_reminder_datetime` writes `ReminderDueBy` as an *unqualified* wall clock precisely so the `TimeZoneContext` decides which instant it means. Give it a `Z` or an offset and the context stops applying — the reminder silently reverts to being stored in UTC whatever zone is sent, which is #8's user-visible symptom coming back by the other door.
- `zoneinfo` needs a timezone database and ships none, so `tzdata` is a declared dependency — unconditionally, because Windows is this server's primary platform and has no system one, which would make the *common* case the silently-degrading one.
