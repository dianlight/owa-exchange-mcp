...
- **Text stability is not a completion signal, and `baseline` alone did not fix that** (2026-09-15). Copilot renders an in-flight status line (`"In corso…"` on it-IT) above an app-promo block, which is *new* relative to the baseline and *unchanging* — so it satisfied both defences and came back as `{"status": "ok"}`. A progress indicator always will: that is what a progress indicator is. Settling therefore keys on the transcript's own turn marker — no `Copilot said:`, no answer — with a fallback path (longer stable run **and** no `_COPILOT_PROGRESS_HINTS` line) so an unlisted localisation degrades to an honest `status: "timeout"` rather than a confident wrong answer. **`_COPILOT_PROGRESS_HINTS` matches whole lines, never substrings**, and that is not tidiness: the summary this bug was hiding contained "In corso" as a status cell inside a table row, so a substring test would reject correct answers. Every signal here is still a localisation table, so the change that would end this bug category rather than patch it is a DOM state attribute marking generation complete — see PROJECT_STATUS.md §4.
