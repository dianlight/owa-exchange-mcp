...
- `EXCHANGE_MCP_DEV_RELOAD` — (optional) Set to `true`/`1`/`yes` to register `dev_reload_tool_modules`, letting a client re-import `exchange_mcp/tools/*.py` on a running `--transport http` server without restarting it — see "Hot reload" below. Same effect as the `--dev-reload` CLI flag. Off by default: dev-only, never for a shared prod instance.
- `EXCHANGE_TIMEZONE` — (optional) The mailbox's own timezone, as a Windows id (`W. Europe Standard Time`) or an IANA one (`Europe/Rome`). **One variable, two consumers, same meaning**: the `TimeZoneContext` every request carries (`mailbox_timezone.py`, issue #8) and the local working-hours grid UTC busy periods are placed on (`availability_frame.py`, #601/#602). Unset — which is the normal case — the zone is read from the mailbox's own OWA configuration once per process; this overrides that, for when the lookup gets it wrong or the server rejects what it returned. The two consumers degrade differently and both say which applied: writes fall back to `UTC` (never a regional zone, see "Timezone" below), while the availability grid falls back to not shifting at all and reports that in the tool's own output. An IANA id always resolves for the grid, so it is also the escape hatch for a Windows id `availability_frame`'s table doesn't know.
...
- `exchange_mcp/` — MCP server package (62 tools)
  ...
  - `auth_errors.py` — Pure diagnosis of a *timed-out* interactive sign-in: reason codes, the AADSTS/page-text/URL hint tables, per-reason remediation text, `AuthenticationRequiredError`, and `SIGNIN_HOST_HINTS`/`looks_like_signin_url()` (what the login poll uses to tell "still signing in" from "worth confirming a session against", without navigating). Imports nothing else from the package (no Playwright) so it stays unit-testable — see "Authentication" below.
  ...
  - `mailbox_timezone.py` — Pure resolution of the timezone id every request carries: the precedence rule (env → mailbox → UTC), the shape guard, the two configuration-response parsers, and the `JsonRequestHeaders`/`TimeZoneContext` builders. No Playwright, unit-testable — see "Timezone" below. Reads the same `GetOwaUserConfiguration` response `mailbox_identity` does, through its own fetcher rather than a shared one; `OWAClient._probe_owa_user_configuration` says why.
  ...
  - `hot_reload.py` — Re-registers one or more `tools/*.py` modules' `@mcp.tool()` functions on a live `MCPServer` without a process restart. Takes the `MCPServer` instance as a parameter rather than importing `server.py` (which owns `mcp` and would create an import cycle). No Playwright, unit-testable — see "Hot reload" below.
...
exchange-mcp-server --transport http --dev-reload  # Same, plus dev_reload_tool_modules (see "Hot reload" below)
...
- **A negative session probe is retried; a positive one is trusted immediately** (`_async_probe_active_session`, `_SESSION_PROBE_ATTEMPTS`). One attempt is not decisive on a cold profile: the first navigation to `/owa/` lands on an account-selection redirect even with a good SSO session, so the bearer capture can give up before the SPA has minted a token — while the request path captures again later and succeeds. That gap logged `NOT AUTHENTICATED` over a perfectly usable session. Same asymmetry as `profile_lock.py`: a false "no" opens a sign-in window nobody asked for, a false "yes" costs one request that already retries.
- **The interactive-login poll must never navigate, because it is polling the page the user is typing into.** `_async_has_active_session()` confirms a modern-backend session by *reloading* the anchor page, so the loop asks the non-destructive `_async_poll_session_signal()` first (canary cookie / a bearer already held / the page having left the sign-in flow per `auth_errors.SIGNIN_HOST_HINTS`) and only pays for the reloading confirmation when that says so, at most once per `_LOGIN_CONFIRM_MIN_INTERVAL_SECONDS`. The host table is one-directional on purpose: matching means "still signing in", but not matching is a *trigger* to check properly, never a verdict that a session exists.
- **Bound the work, then derive the wrapper budget from it — never the reverse.** `interactive_login()`'s old `timeout + 30` margin exactly equalled the post-timeout diagnosis's worst case (10 `VISIBLE_ERROR_SELECTORS` × 3 nodes × two 500 ms calls), so the diagnosis ate it and `future.result()` raised a `concurrent.futures.TimeoutError` — which *is* the builtin `TimeoutError`, and its `str()` is empty, hence a log line reading literally `TimeoutError: .` with no reason and no remediation. The diagnosis is capped (`_LOGIN_DIAGNOSIS_SECONDS`) and degrades to the plain timeout result rather than replacing the answer with an exception; `_LOGIN_RUN_MARGIN_SECONDS` and `has_active_session()`'s default are computed from the bounded parts. **Any new post-deadline work belongs in that sum on the same commit** — this regressed twice, the second time while the fix was being written. `tests/unit/test_login_probe.py` asserts it as an invariant.
...
**Own mailbox address — always through `OWAClient.resolve_own_mailbox()` / `mailbox_address()`, never a stored field.** There is no signed-in username to remember: authentication is a browser profile, so *who we are* has to be discovered from the session. `resolve_own_mailbox()` does it once per process from three signals in descending order of authority — `x-anchormailbox` (captured with the bearer token, no request), `GetOwaUserConfiguration` (one request), then Bearer-JWT claims — and caches the result, including a failure (the realistic failure is a backend with no such surface, and re-probing would add a request to every availability call forever). **A cached failure is not the same kind of thing as a cached address, and conflating them was a bug.** `UNRESOLVED_NO_ADDRESS` ("signals were there and carried no address") is a property of the backend and stays cached for the process. `UNRESOLVED_NO_SIGNALS` ("nothing had answered yet") is a statement about *when* we asked, so it is re-asked when `mailbox_identity.signal_state()` moves — and only then. Caching it outright pinned `find_free_time` to its recurrence-blind fallback for the life of a process, which under `stdio` is one client session (fixed 2026-09-16). Re-probing on every call while unresolved is the opposite error and measurably worse: the configuration probe is flaky *per session* (four cold processes — two first-call resolutions, 0-for-6 on further attempts in the other two), so it costs ~30s a call and never wins. §4 has the numbers, including the two alternatives that were tried and rejected. `login(force=True)` is the only event that can change the answer, so it calls `forget_mailbox_identity()`. The parsing lives in `mailbox_identity.py`; add signals to its tables and cover them in `tests/unit/test_mailbox_identity.py`, not in `owa_client.py`. Three rules are load-bearing, all of them paid for by the six days (2026-09-10 to 2026-09-16) when the predecessor field `user_email` had five readers and no writer (PROJECT_STATUS.md §4):
...
- **Only the request-free *authoritative* signal may short-circuit the ordering.** `resolve_own_mailbox()` deliberately withholds the bearer token from its first pass, even though reading it costs nothing: a `upn` claim would otherwise answer there and the configuration — which outranks it — would never be fetched, inverting the documented priority on every bearer-mode session. `resolve_mailbox_address()` orders the signals correctly and a unit test pinned that; the bug was in the *caller*, which never supplied the higher-priority input, and only a live run caught it (`get_meeting_contacts` resolving from `bearer_claim:upn` on a mailbox whose configuration answers fine). A pure-function test can pin a priority order and still miss that.
...
**The address and the timezone come out of one `GetOwaUserConfiguration` response.** Both of the two sections above resolve a per-mailbox fact from the same blob, and they were built in parallel (PRs #27 and #28), each with its own probe method issuing the same request — a second round-trip on the first availability call for something that cannot change underneath us. `OWAClient._owa_user_configuration()` is now the single call site, cached for the process, and both read from it; whichever question is asked first pays. Three things are load-bearing if you touch it:
- **It caches successes only, and lets exceptions out.** Each caller already caches its own *outcome*, so a backend with no such surface costs at most one failed request per caller and never repeats per tool call. Caching the exception at the shared layer would force one caller's error semantics on both, and they differ deliberately: the timezone path must see `SessionExpiredError` distinctly ("asked before sign-in landed, try again"), while the identity path swallows everything and degrades to `""`. `_user_configuration_or_none()` is the identity side's swallowing wrapper, and that is *why* the cache sits below it rather than in it.
- **The delegation direction is the whole trick.** The two probes were originally kept apart precisely because delegating the timezone one to the identity one would have made a `SessionExpiredError` indistinguishable from "this backend won't serve it". That reasoning was right; the fix is to delegate the other way, so the shared fetcher surfaces expiry and the swallowing lives in a thin wrapper above it.
- **`forget_mailbox_identity()` has to clear all three caches**, not just the address. Clearing the address alone is worse than doing nothing: the next read re-resolves it straight out of the previous account's still-cached blob, so a stale address returns looking freshly probed. `tests/unit/test_mailbox_identity.py` asserts the re-probe by request *count*, which is what catches this — a value check cannot.
- **Clearing them is still not enough on its own**, so `forget_mailbox_identity()` bumps `_identity_generation` first and every cache writer discards its result if that marker moved while its request was in flight. Each cache is written *after* its request returns, so a clear landing in between was simply overwritten by the previous account's answer — reproduced: after switching mid-probe, the new account kept the old one's address *and* zone, looking freshly resolved. A marker rather than the two existing locks because `resolve_own_mailbox()` holds neither while it probes (nothing to serialise against), and taking them would block `login` behind a round-trip. Add a new per-mailbox cache → guard it the same way, and cover it in `tests/unit/test_identity_cache_generation.py`, whose fake transport blocks mid-request so the interleaving is deterministic.
...
**Hot reload (`hot_reload.py`, `--dev-reload`/`EXCHANGE_MCP_DEV_RELOAD`)**: a long-lived `--transport http` process is exactly the case where restarting to pick up an edit is expensive — it drops the bound port (every connected client must reconnect) and, depending on session freshness, can force a fresh interactive sign-in. `dev_reload_tool_modules(modules=[...])` re-imports one or more `exchange_mcp/tools/*.py` modules on the running process instead, with no restart and no new browser/login. This is a developer calling a tool after saving an edit, not a background file watcher — nothing reloads on its own. Scope is exactly the 11 domain tool modules; anything in `owa_client.py`, `browser_session.py`, `server.py` itself, or any other shared module still needs a real restart, because reload only re-executes the one target module, not its dependencies or module-level globals living elsewhere. Three things are load-bearing:

- **The swap is remove-then-reload-then-re-add, never a bare `importlib.reload()`.** `MCPServer.add_tool()` silently *keeps the old tool* on a name collision rather than raising, so reloading a module whose tools are still registered under their old names would just re-run the decorators into a no-op. `hot_reload.reload_modules()` therefore removes every tool currently attributed to a module (matched by `Tool.fn.__module__`) before reloading it, so the fresh `@mcp.tool()` calls register cleanly.
- **A reload that raises rolls the whole module back, not just the well-behaved part of it.** Some of a broken module's `@mcp.tool()` calls may already have run before the exception — those partial registrations are removed too, then every tool the module had *before* the attempt is re-added from the saved `Tool` objects (same `fn`, same metadata). A previously-working tool is never left unregistered because a later edit to the same file broke.
- **`--stable`'s exclusions don't survive a plain reload, so `reload_modules()` re-applies them.** `_apply_stable_mode()` sets a module-level `_stable_mode_active` flag; every reload checks it and strips any reloaded tool that's still in `KNOWN_BUGGY_TOOLS`, recording it as `stable_excluded`. Without this a reload of a module containing an excluded tool would quietly re-expose it to a `--stable` client.

The whole remove→reload→re-add sequence is synchronous with no `await` in it, which is what makes it atomic with respect to the single-threaded asyncio loop — no other tool call can observe a half-swapped registry. Registered conditionally inside `main()` (`if args.dev_reload: mcp.add_tool(dev_reload_tool_modules)`), exactly mirroring how `--stable` is applied, rather than always defining it with `@mcp.tool()` and trying to remove it later — an ASGI embedder that serves `mcp` directly without going through `main()` already doesn't get `--stable` filtering either, so this introduces no new inconsistency. Not counted in the "62 tools" inventory or `PROJECT_STATUS.md`: it's an operational/dev utility, not a mailbox capability, and only exists at all when the flag is set.
