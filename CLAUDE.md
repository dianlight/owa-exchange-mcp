...
- **The classic-OWA `GetUserAvailability` branches cannot be reached from any live run** (it answers a permanent `NotImplementedException` here, #602), so they are covered by `tests/unit/test_availability_frames.py` instead — a fake transport that asserts the *payload* (`TimeZoneContext` id, `TimeWindow`) as well as the parsed frame, the same technique as `test_folder_resolution.py`. Any change to a busy-period parser or to a wire timezone belongs there: the guard that matters is that an *unqualified* `CalendarEvent` time is already wall clock in the `TimeZoneContext` we sent, so `from_utc()` on that path double-shifts it into a bug indistinguishable from #601. That suite's mutation-checked assertions reproduce the original bug's exact numbers when the conversion is reverted. Note the remaining `"Russian Standard Time"` literals in `calendar.py`/`tasks.py`/`folders.py` are **not** yet migrated: those modules have their own documented conventions (tasks.py explicitly documents UTC+3 reminder behaviour) and moving them is a separate, separately-verifiable change.
