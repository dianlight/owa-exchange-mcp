...
- The probe is `OWAClient.mailbox_timezone()` — `GetOwaUserConfiguration`, once per process, cached like `user_email`, and allowed to fail quietly. Add new timezone ids to `WINDOWS_TO_IANA` and new response spellings to `TZ_ID_KEYS`, not to the walker, and cover them in `tests/unit/test_mailbox_timezone.py`.
