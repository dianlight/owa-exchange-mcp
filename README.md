...
| `sync_folder_items` | Delta sync a folder's changes since a `sync_state` token (or a first full sync with none) |
...
| `create_folder` | Create a folder — a mail folder by default, or a Microsoft To Do list with `folder_class="IPF.Task"` under `parent_folder_id="tasks"` |
